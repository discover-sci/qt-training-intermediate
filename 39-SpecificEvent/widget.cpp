...
void Widget::mousePressEvent(QMouseEvent *event) {
  qDebug() << "\n=== Mouse Press Event ===";
  //int ret = QMessageBox::warning(this, "Title of a message...", "Mouse pressed @");
...
void Widget::closeEvent(QCloseEvent *event) {
  QMessageBox::information(this, "Just to say goodby...", "Thanks for using our app.");
...
void Widget::contextMenuEvent(QContextMenuEvent *event) {
  ...
  //Create a menu with parent
  QMenu menu(this); // right click and menu pops-up
  menu.addAction("Action1");
  menu.addAction("Action2");
  // menu.exec(mapToGlobal(event->pos())); // location where to pop up the menu. Needs mapping to global position coordinates, not local position
                                          // must be mapToGlobal() and not event->globalPos(), since there might be margins in the widget...
