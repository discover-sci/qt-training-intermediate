...
void PersonModel::addPerson(Person *person) {
  ...
  QModelIndex idx1 = this->index(persons.size()-1, 0); // index returns the QModelIndex in last row: persons.size()-1 and column 0
  QModelIndex idx2 = this->index(persons.size()-1, 1); // index returns the QModelIndex in last row: persons.size()-1 and column 1
  QModelIndex idx3 = this->index(persons.size()-1, 2); // index returns the QModelIndex in last row: persons.size()-1 and column 2

  this->setData(idx1, person->names(), Qt::EditRole);
  this->setData(idx2, person->favoriteColor(), Qt::EditRole);
  this->setData(idx3, person->age(), Qt::EditRole);
...
Qt::ItemFlags PersonModel::flags(const QModelIndex &index) const {
  if (!index.isValid())
    return QAbstractTableModel::flags(index);  // call default flags (which is Qt::NoItemFlags)

  return QAbstractTableModel::flags(index) | Qt::ItemIsEditable; // otherwise all what is by default and enable "editability"
