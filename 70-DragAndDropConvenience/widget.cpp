...
Widget::Widget (QWidget *parent) : QWidget (parent), ui (new Ui::Widget) {
  ...
  ui->listWidget->setDragDropMode(QAbstractItemView::InternalMove); // Move the data, not copy as by default
  ...
  for(int row = 0; row<rows ; ++ row){
    ...
    for(int col=0; col<columns; ++col){
      ...
      ui->tableWidget->item(row,col)->setData(Qt::ToolTipRole,QString("item [%1,%2]").arg(row).arg(col));
...
void Widget::newRow() {
  int row = ui->tableWidget->rowCount(); // obtain number of rows
