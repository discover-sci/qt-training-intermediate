...
// accepts child pointer, but its moved - call this with move oepration
...
Person *Person::child(int row) {
  ...
  return this->children[row].get(); // access to the raw pointer... the caller of the obtained Person* in this case will not be the owner.
}
...
bool Person::insertChildren(int position, int count, int columns) {
  ...
  for(int row = 0; row < count; ++ row){
    ...
    this->children.insert(this->children.begin() + position, std::move(person));
...
bool Person::removeChildren(int position, int count) {
  ...
  if (position < 0 || position + count > static_cast<int>(children.size()))
    return false;
  children.erase(this->children.begin() + position, this->children.begin() + position + count);
  return true;
}
