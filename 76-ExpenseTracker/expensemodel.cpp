...
QVariant ExpenseModel::data(const QModelIndex &index, int role) const {
  if (!index.isValid() || index.row() >= expenses_.count())
    return QVariant();

  Expense *expense = expenses_[index.row()]; // find domain class representative object
...
bool ExpenseModel::setData(const QModelIndex &index, const QVariant &value, int role) {
  ...
  Expense *expense = expenses_[index.row()];
  bool changed = false;
...
void ExpenseModel::addExpense(Expense *expense) {
  int row = expenses_.size();
  beginInsertRows(QModelIndex(), row, row);
  expenses_.append(expense);
  endInsertRows();
...
void ExpenseModel::removeExpense(const QModelIndex &index) {
  ...
  delete expenses_.takeAt(index.row());
  endRemoveRows();
  saveExpenses(); // Save happens automatically
}
...
bool ExpenseModel::loadExpenses() {
  ...
  qDeleteAll(expenses_);  // delete all pointers in a sense of delete() call on each pointer
  expenses_.clear();      // no also remove pointers
